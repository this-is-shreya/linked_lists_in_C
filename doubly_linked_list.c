...
for(temp=FIRST;temp!=LAST;temp=temp->NEXT){
    if(temp->value==ele){
            if(temp!=FIRST)
        printf("%d is present between %d and %d in the list\n", ele, (temp->PREV)->value, (temp->NEXT)->value);
            else
                printf("%d is present between null and %d in the list\n", ele, (temp->NEXT)->value);
        count=temp;
        break;
    }
}
if(LAST->value==ele)
{
printf("%d is present between %d and %s in the list\n", ele,(LAST->PREV)->value,"null");
    count=LAST;
...
printf("enter 6 to print in reverse\n");
int choice;
scanf("%d",&choice);
switch(choice){
